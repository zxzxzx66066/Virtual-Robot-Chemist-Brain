...
本项目是一个面向 **AI for Science (数字孪生/自动化实验室)** 赛道的软硬结合高并发调度系统。旨在通过大语言模型 (LLM) 与高频异步网络通信，实现从自然语言指令到多端底层硬件（光电传感器、工业机械臂）的自动化编排与无阻塞执行。即虚拟数字自动化实验室调度系统(Virtual Robot Chemist Brain)，模拟接收自然语言指令，自动拆解实验步骤，并异步调度各个“虚拟设备”完成实验的全过程
